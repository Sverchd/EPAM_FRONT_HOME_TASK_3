...
body[0].appendChild(firstSection);
let catalog = document.createElement("div");
catalog.className = "catalog";
firstSection.appendChild(catalog);
let device = document.createElement("div");
device.className = "device";
let imgBlock = document.createElement("div");
imgBlock.className = "imgBlock";
let deviceImage = document.createElement("img");
deviceImage.className = "deviceImage";
deviceImage.src = "img/smartphone.jpg";
deviceImage.alt = "smartphone";
...
deviceName.innerHTML = data[0].name;
let reviews = document.createElement("p");
reviews.className = "Reviews";
reviews.innerHTML = data[0].ratingRevievs;
catalog.appendChild(device);
device.appendChild(imgBlock);
device.appendChild(dataBlock);
imgBlock.appendChild(deviceImage);
dataBlock.appendChild(deviceName);
let oldPrice = document.createElement("p");
let newPrice = document.createElement("p");
oldPrice.setAttribute('name', 'price');
newPrice.className = "newPrice";

if (data[0].price.newUan == undefined) {
    oldPrice.className = "singlePrice";
    oldPrice.innerHTML = data[0].price;
} else {
    oldPrice.className = "oldPrice";
    oldPrice.innerHTML = data[0].price.oldUan;

    newPrice.innerHTML = data[0].price.newUan;
}

dataBlock.appendChild(reviews);
dataBlock.appendChild(oldPrice);
dataBlock.appendChild(newPrice);

for (let i = 1; i < data.length; i++) {
    let deviceclone = device.cloneNode(true);
    deviceclone.getElementsByClassName("deviceName")[0].innerHTML = data[i].name;
    deviceclone.getElementsByClassName("Reviews")[0].innerHTML = data[i].ratingRevievs;
    let newprice = deviceclone.getElementsByClassName("newPrice")[0];
    let test = document.getElementsByName("price");
    if (data[i].price.newUan == undefined) {
        newprice.innerHTML = "";
        catalog.appendChild(deviceclone);
        let price = document.getElementsByName("price")[i];
        ...
        price.innerHTML = data[i].price.oldUan;
    }
}
